...
const TvShowsPage = () => {
  ...
  return (
    <div className="container" style={{ marginTop: '6rem', padding: '2rem' }}>
      <div className="search" style={{ marginBottom: '2rem', marginTop: '2rem', display: 'flex' }}>
        <Input type="text" onChange={(e) => setQuery(e.target.value)} placeholder="Search TV Shows" style={{ width: '80%' }} />
        <button className='btn-search' onClick={() => handleSearch(query)} style={{ marginLeft: '1rem' }}>
          Search
        </button>

      </div>

      {searchShows.length > 0 && (<MovieGrid items={searchShows} category={category.tv} />)}
      <h2 id='no-results' style={{ textAlign: 'center', marginTop: '2rem', marginBottom: '2rem', display:'none' }}>No results found</h2>

      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
        <button onClick={() => setActiveSection('popular')} className={`${activeSection === 'popular' ? 'active' : '' } categ`} >Popular</button>
        <button onClick={() => setActiveSection('top_rated')} className={`${activeSection === 'top_rated' ? 'active' : '' } categ`}>Top Rated</button>
        <button onClick={() => setActiveSection('on_the_air')} className={`${activeSection === 'on_the_air' ? 'active' : '' } categ`}>On The Air</button>
      </div>
